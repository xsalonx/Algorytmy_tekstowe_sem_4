...
def search_2d_pattern(text: str,
                      patterns,
                      FIRST_CHAR=48):  # patterns - list of 2d patterns; each given as "<line1>\n<line2>\n<line3>..."
    ...
    pos_search_patterns = ["".join([chr(FIRST_CHAR + subpatterns_searcher.pattern_idx(p)) for p in pc]) for pc in
                           Patterns]
...
def search_2d_image_pattern(im_path, pattern_paths, FIRST_CHAR=48):
    ...
    image_text = "\n".join(
        ["".join([chr(image[j, i] + FIRST_CHAR) for i in range(image.shape[1])]) for j in range(image.shape[0])])
    pattern_text = [
        "\n".join(["".join([chr(p_im[j, i] + FIRST_CHAR) for i in range(p_im.shape[1])]) for j in range(p_im.shape[0])])
        for p_im in pattern_ims]
...
def main_im(whole_text_im_file_path, patter_im_file_paths, show_each_ad=False):
    ...
    Xv = defaultdict(lambda: [], [])
    Yv = defaultdict(lambda: [], [])
...
if __name__ == '__main__':
    ...
    patters_im_file_paths = ["data/pattern.png", "data/algorithms.png", "data/of.png"]
