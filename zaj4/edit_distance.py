...
def printOperationsInOneString(D, i, j, str1, str2):
    if i > 0 and j > 0:
        if str1[i - 1] == str2[j - 1]:
            return printOperationsInOneString(D, i - 1, j - 1, str1, str2) + f"{str1[i - 1]}"
        else:
            m = min(D[j - 1][i], D[j][i - 1], D[j - 1][i - 1])
            if m == D[j][i - 1]:
                return printOperationsInOneString(D, i - 1, j, str1, str2) + f"(-{str1[i - 1]})"
            elif m == D[j - 1][i]:
                return printOperationsInOneString(D, i, j - 1, str1, str2) + f"(+{str2[j - 1]})"
            else:
                return printOperationsInOneString(D, i - 1, j - 1, str1, str2) + f"({str1[i - 1]}->{str2[j - 1]})"
...
if __name__ == '__main__':
    ...
    for s1, s2 in zip(str1, str2):
        ...
        print(f"str1: {s1}\nstr2: {s2}")
        opsInString = printOperationsInOneString(D, len(s1), len(s2), s1, s2)
        print(opsInString)
